...
- **ZCode 路由与结果解析修复**：ZCode 任务不再被错误迁移到网关，兼容 CLI 的多行 JSON 终态输出，并隔离其缓存 Cohort。
- **双 Harness 委派校验**：批量委派、自动规划子任务和管理页均保留 `harness`，并显示 ZCode Provider 套餐状态（不返回密钥）。
...
从 v0.7.1 Release 直接安装已验证的通用 Wheel 包：
